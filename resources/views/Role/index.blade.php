...
@include('Layout.side')
                    ...
                    >
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="inputData" class="form-label">Nama Role</label>
                                    <input
                                        type="text"
                                        class="form-control"
                                        id="inputData"
                                ...
                                <div class="mb-3" hidden>
                                    <label for="inputData" class="form-label">guard_name</label>
                                    <input
                                        type="text"
                                        class="form-control"
                                        id="inputData"
                    ...
                    <div class="card-body px-0 pb-0">
                        <div class="table-responsive">
                            <table class='table mb-0' id="table1">
                                ...
                                <tbody>
                                    @php ($counter = 1) @foreach ($role as $r)
                                
                                <tr>
                                    <td>{{ $counter }}</td> 
                                    <td>{{ $r->name }}</td>
                                    <td>
                                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#editModal{{$r->id}}">Edit</button> <div class="modal fade" id="editModal{{$r->id}}" tabindex="-1" aria-labelledby="editModal{{$r->id}}Label" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editModal{{$r->id}}Label">Edit Kategori</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <!-- Form for editing data -->
                    <form action="{{ url('Role-Update/'.$r->id) }}" method="POST">
    @csrf
    @method('PUT')
                        <div class="mb-3">
                       
                            <input type="text" class="form-control" id="editUser{{$r->name}}" name="name" value="{{$r->name}}" >
                        </div>
                        <div class="mb-3">
                  
                            <input type="text" class="form-control" id="editEmail{{$r->guard_name}}" name="guard_name" value="web" >
                        </div>
                        ...
                        <button type="submit" class="btn btn-primary">Save Changes</button>
                                       ...
                                       <a href="{{ route('Role-Delete', $r->id) }}"class="btn btn-danger delete-btn"
    ...
    <form id="delete-form-{{$r->id}}" action="{{ route('Role-Delete', $r->id) }}" method="POST" style="display: none;">
        @csrf
        @method('DELETE')
    </form>
                                    </td>
